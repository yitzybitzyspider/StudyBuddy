...
def test_session_pulls_new_items_when_nothing_due(tmp_path):
    ...
    store.save_learner(LearnerState(learner_id=store.DEFAULT_LEARNER), subject="finance", root=tmp_path)
...
def test_record_session_grades_reschedules_and_tracks_progress(tmp_path):
    ...
    # answer it correctly
    path = store.doc_path(store.DEFAULT_LEARNER, "finance", execute.SESSION_NAME, root=tmp_path)
