...
|---|----------|--------|-----------|
| A1 | Language & validation | Python 3.11+. **Pydantic v2** is the source of truth for the data-model entities; the **per-call output contracts** are JSON Schema stored in `prompts/` (entity-shaped ones exported from the Pydantic models, verdict/score-shaped ones hand-authored). The wrapper validates Claude output against the call's `output_schema` with the **`jsonschema`** validator. Plain-text JSON Schema keeps contracts diffable. | build-plan suggested stack (Python); spec §5/§6 (validate output against the call's schema); philosophy §2 |
...
| A6 | Canonical reference shape | One reusable `Reference {kind: material\|web\|item\|run\|prompt, ref, locator}` used by every `source_refs` / `source_links` / `evidence_refs` / item provenance / run-log pointer. | NFR-3 (source-linkability); philosophy §11 (everything traces to source) |
