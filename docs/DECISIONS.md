...
|---|----------|--------|-----------|
| A1 | Language & validation | Python 3.11+, Pydantic v2 for the data model **and** Claude-output validation. Per-call JSON Schema is exported from the models and stored in `prompts/` so contracts stay diffable. | build-plan suggested stack (Python); philosophy §2 (diffable plain-text contracts) |
